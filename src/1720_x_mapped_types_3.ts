...
type ValidatorFactory2<O> = {
  [K in keyof O & string as `validate_${K}`]: (
    valueToValidate: O[K],
    key: K,
    object: O
  ) => boolean;
...
declare function makeValidator2<O extends object>(o: O): ValidatorFactory2<O>;

const r2 = makeValidator2(person);

r2.validate_age(22, "age", person);
r2.validate_firstname(22, "firstname", person); // ERROR
